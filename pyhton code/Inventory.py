...
def add_item(item, quantity):
    ...
    if item in inventory:# 1. Check if the item exists in the inventory dictionary.
        inventory[item]=inventory[item] + quantity# 2. If it does, increase its quantity.
    else:
        inventory[item]=inventory[item] = quantity # 3. If not, add the item to the inventory with the given quantity.
    ...
    while True:
        ...
        if choice == "1":
            ...
            try:
                ...
                print("error detected: please use numbers")
                quantity=0
